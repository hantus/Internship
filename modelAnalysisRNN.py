...
def getData(testSize):
    X = np.load('data/rnn/dataUnbalanced.npy')
    X = np.reshape(X, (X.shape[0],10, 64))

    y = np.load('data/rnn/labelsUnbalanced.npy')
...
allIN = np.count_nonzero(y_test == 0)
...
for i in range(total):
    if y_test[i] != np.argmax(y_pred[i]):
        if (y_test[i] == 0) & (np.argmax(y_pred[i]) == 2):
            grINprOUT += 1
        elif (y_test[i] == 0) & (np.argmax(y_pred[i]) == 1):
            grINprX += 1
        elif (y_test[i] == 2) & (np.argmax(y_pred[i]) == 0):
            grOUTprIN += 1
        elif (y_test[i] == 2) & (np.argmax(y_pred[i]) == 1):
            grOUTprX += 1
        elif (y_test[i] == 1) & (np.argmax(y_pred[i]) == 0):
            grXprIN += 1
        elif (y_test[i] == 1) & (np.argmax(y_pred[i]) == 2):
            grXprOUT += 1
