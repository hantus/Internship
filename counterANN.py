...
class TrackedCluster:
    ...
    def __init__(self, id, x, y, side):
        ...
        self.assigned = True
        self.side = side
print(f"the size of trackedCluster is {getsizeof(TrackedCluster)}")
...
print(f"the size of IdTracker is {getsizeof(IdTracker)}")
...
frames = data.shape[0]
trackedClusters = []
print(f"the size of a list is {getsizeof(trackedClusters)}")
idTracker = IdTracker()
print(f"the size of id track instance is {getsizeof(idTracker)}")
print(f"the size of threshold var is {getsizeof(threshold)}")
people = 0

# load neural model
model = load("data/models/model-70-100.joblib")
print(f"the size of the model is {getsizeof(model)}")
...
for i in range(startFrame, frames):
    ...
    print(f"the size of data is {getsizeof(data[i])}")
 ...
 # assign found clusters to tracked clusters
    if len(clusters) > 0:
        for cl in clusters:
            print(f"the size of Cluster is {getsizeof(cl)}")
            ...
            # else create a new tracked cluster
                ...
                if(cl.points == 1) & (cl.y > 2) & (cl.y < 5):
                    ...
                    print(f"the size of new tracked cl is {getsizeof(newTrackedCluster)}")
